...
func (team *Team) MuteTeamList(tid string, owner string, accid string, mute int) (*BaseResp, error) {
	res, err := ResponseResult(team.APPKEY, team.APPSECRET, ACTION_TEAM_MUTE_LIST, url.Values{"tid": {tid}, "owner": {owner}, "accid": {accid}, "mute": {strconv.Itoa(mute)}})
	if err != nil {
		return nil, err
	}
	result := &BaseResp{}
...
func (team *Team) MuteTeamListAll(tid string, owner string, mute bool, muteType int) (*BaseResp, error) {
	res, err := ResponseResult(team.APPKEY, team.APPSECRET, ACTION_TEAM_MUTE_ALL, url.Values{"tid": {tid}, "owner": {owner}, "mute": {strconv.FormatBool(mute)}, "muteType": {strconv.Itoa(muteType)}})
	if err != nil {
		return nil, err
	}
	result := &BaseResp{}
